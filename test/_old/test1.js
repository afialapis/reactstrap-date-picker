...
describe('reactstrap-date-picker ...', function () {
  this.timeout(1000)
  ...
  it('should render a date picker', () => {
    ...
    expect(wrapper.find(`input#${did}`).length).to.equal(1)
    wrapper.unmount()
  })
  ...
  it("should render a date picker with a value.", () => {
    ...
    const hiddenInputElement= wrapper.find(`input#${did}`).getDOMNode()
    const hiddenInputFmtValue = hiddenInputElement.getAttribute('data-formattedvalue')
  ...
  it("should open the calendar and select a date.", () => {
    const did = 'basic-alendar-select'
    const Unit = () => 
      <DatePicker id={did} />
    ...
    const hiddenInputElement= wrapper.find(`input#${did}`).getDOMNode()
    expect(hiddenInputElement.getAttribute('value')).to.equal('')
    expect(hiddenInputElement.getAttribute('data-formattedvalue')).to.equal('')

    const inputElement = wrapper.find(`input.form-control`) // document.querySelector("input.form-control")
    

    inputElement.simulate('focus')
    const dayElement = wrapper.find(`table tbody tr:last-child td`).at(0) //.getDOMNode()
    dayElement.simulate('click')


    expect(hiddenInputElement.getAttribute('value')).to.not.equal('')
    expect(hiddenInputElement.getAttribute('data-formattedvalue')).to.not.equal('')
  ...
  */
  ...
  it("should show next and prev buttons if min and max dates are set but not displayed.", () => {
    const id = UUID.v4()
    const displayDate = "2017-07-21T12:00:00.000Z"
